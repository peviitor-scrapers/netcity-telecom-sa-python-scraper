...
Python scraper that reads job listings from the NETCITY TELECOM S.A. applytojob
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/netcity-telecom-sa-python-scraper --json status -q .status
```
...
(`gh issue list --repo peviitor-scrapers/netcity-telecom-sa-python-scraper --state open`),
...
python3 -m scraper.validate_jobs 22902080 --mode head  # validate job URLs
```
