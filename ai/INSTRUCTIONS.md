...
This scraper extracts job listings for **NETCITY TELECOM S.A.** (CIF `22902080`)
from the group's applytojob board, filtered by the `NETCITY` department,
and publishes them to peviitor.ro via the v1 API (`https://api.peviitor.ro/v1`).

Target: `https://electrogrup.applytojob.com/apply/jobs/?department=NETCITY`
...
1. **Start with brand** — `NETCITY`
2. **Search in ANAF/CUIScan** — find company by CIF `22902080`
...
7. **Scrape new jobs** — parse the NETCITY applytojob board (department filter)
...
4. **Scrape jobs** — parse the NETCITY applytojob board
...
company.py / anaf.py (validate company)
    ...
    │
    ▼ (if active)
scrape applytojob board (?department=NETCITY)
    │
    ▼
...
- **Applytojob board**: `https://electrogrup.applytojob.com/apply/jobs/?department=NETCITY` — listing HTML
...
python3 -m scraper.api 22902080

# Validate job URLs from peviitor by CIF (head/content/browser)
python3 -m scraper.validate_jobs 22902080 --mode head
python3 -m scraper.validate_jobs 22902080 --mode content --dry-run
python3 -m scraper.validate_jobs 22902080 --mode content --delete
```
