"""End-to-end test: scrape the real NETCITY applytojob board.

The NETCITY group shares the electrogrup.applytojob.com board; this scraper
filters to the `NETCITY` department. Skips (rather than fails) when the board
...
# Observed: 0 unique jobs for the NETCITY department at time of writing.
...
def test_scrape_real_board():
    ...
    assert "NETCITY" in index.DEPARTMENT, "scraper should target the NETCITY department"
