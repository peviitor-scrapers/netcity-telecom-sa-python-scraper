...
<html><body>
<div class="row">
  <a class="job_title_link" href="/apply/jobs/details/ABC123?department=NETCITY">Inginer Ofertare Energetic</a>
</div>
<div class="row">
  <a class="job_title_link" href="/apply/jobs/details/DEF456?department=NETCITY">SCADA Engineer</a>
</div>
</body></html>
"""
...
<html><body><table>
  <tr>
    <td><a class="job_title_link" href="/apply/jobs/details/AAA111?department=NETCITY">Job A</a></td>
    <td>Bucuresti, Bucuresti, Romania</td>
  </tr>
  <tr>
    <td><a class="job_title_link" href="/apply/jobs/details/BBB222?department=NETCITY">Job B</a></td>
...
def test_map_to_job_model_adds_company_and_status():
    ...
    index.COMPANY_NAME = "NETCITY TELECOM S.A."
    job = index.map_to_job_model(raw, "22902080")
    assert job["company"] == "NETCITY TELECOM S.A."
    assert job["cif"] == "22902080"
...
def test_transform_jobs_for_solr_keeps_required_fields():
    jobs = [{"url": "https://x/job", "title": "Test Job", "location": ["Cluj-Napoca"],
             "company": "NETCITY TELECOM S.A.", "cif": "22902080"}]
    transformed = index.transform_jobs_for_solr({"company": "NETCITY TELECOM S.A.", "jobs": jobs})
    assert len(transformed["jobs"]) == 1
    t = transformed["jobs"][0]
    assert t["url"]
    assert t["title"]
    assert t["location"] == ["Cluj-Napoca"]
    assert t["company"] == "NETCITY TELECOM S.A."
...
def test_main_dry_run_writes_summary(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(index, "validate_and_get_company", lambda: {
        "company": "NETCITY TELECOM S.A.", "cif": "22902080", "status": "active",
        "address": "CLUJ-NAPOCA"})
